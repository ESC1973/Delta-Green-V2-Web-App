                ...
                {characters.map((char, index) => (
                    <div key={index} className="flex flex-col items-center text-center">
                        <div className="w-24 h-24 rounded-full overflow-hidden border-2 border-gray-600 mb-2 shadow-lg">
                            <img src={char.imageUrl} alt={char.name || 'Unknown Agent'} className="w-full h-full object-cover" />
                        </div>
                        <p className="font-bold text-sm text-gray-200">{char.name || 'Unknown Agent'}</p>
                    </div>
                ))}
            </div>
        </div>
