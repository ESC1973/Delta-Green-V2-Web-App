...
const App: React.FC = () => {
  ...
  const processResponse = (response: HandlerResponse | null) => {
    if (response) {
  ...
  useEffect(() => {
    if (gameState === 'playing' && gameLog.length === 0 && !isLoading) {
      const startGame = async () => {
        setIsLoading(true);
        try {
          const response = await generateHandlerResponse([], context, characters);
          processResponse(response);
        } catch (error) {
  ...
  }, [gameState, gameLog.length, isLoading, context, characters, processResponse]);
      ...
      <div className="h-full w-full bg-black bg-opacity-70 flex flex-col">
        ...
        {gameState === 'setup' ? (
          ...
          <div className="flex-grow flex flex-col md:flex-row overflow-hidden">
            <div className="w-48 bg-black bg-opacity-20 p-4 border-r border-gray-800 hidden md:block overflow-y-auto">
